...
for i in range(1,31):
    xtrain, xtest, ytrain, ytest = train_test_split(x,y,test_size = 0.25, random_state=i)#the test size is the % of data used to test the model, the rest of the data
...
def PlotDrawer(Plot_one,Plot_Two,X_axis):
    ...
    plt.legend()
    plt.show()
PlotDrawer(trainacc,testacc,DifferentK)#right is underfit (train and test acc are close(higher k)) left is overfit (train and test acc are very diff(lower k))
